...
<div class="p-6 w-full flex flex-col space-y-6 items-center">
    ...
    <form id="memberForm" action="#" method="POST" class="max-w-4xl mx-auto">
        @csrf
        ...
        <div id="section1" class="bg-white rounded-xl shadow-sm border border-gray-100 mb-6">
            <div class="p-6 border-b border-gray-100">
                ...
                <p class="text-gray-600 text-sm mt-1">Edite os dados pessoais básicos do crente</p>
            </div>
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="nome"
                            ...
                            placeholder="Ex: João Baptista Silva" value="{{ old('nome', $crente->nome) }}">
                        @error('nome')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="date" name="data_nascimento"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition"
                                value="{{ old('data_nascimento', $crente->data_nascimento ? date('Y-m-d', strtotime($crente->data_nascimento)) : '') }}">
                        ...
                        <div>
                            ...
                            <input type="tel" name="telefone"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition"
                                placeholder="+258 84 123 4567" value="{{ old('telefone', $crente->telefone) }}">
                            @error('telefone')
                                <div class="text-red-600 text-sm mt-1">
                                    <i class="fas fa-exclamation-circle mr-1"></i>
                                    <span>{{ $message }}</span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="nome_pai"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition"
                                placeholder="Nome completo do pai" value="{{ old('nome_pai', $crente->nome_pai) }}">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Nome da Mãe</label>
                            <input type="text" name="nome_mae"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition"
                                placeholder="Nome completo da mãe" value="{{ old('nome_mae', $crente->nome_mae) }}">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="profissao"
                                ...
                                value="{{ old('profissao', $crente->profissao) }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="endereco"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition"
                                placeholder="Ex: Muatala, Nampula" value="{{ old('endereco', $crente->endereco) }}">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <label
                            class="border-2 border-gray-200 rounded-xl p-4 cursor-pointer hover:border-red-300 transition-colors {{ (old('genero', $crente->genero) == 'masculino') ? 'border-red-500 bg-red-50' : '' }}"
                            onclick="selectRadioCard(this, 'genero')">
                            <input type="radio" name="genero" value="masculino" class="hidden"
                                {{ (old('genero', $crente->genero) == 'masculino') ? 'checked' : '' }}>
                        ...
                        <label
                            class="border-2 border-gray-200 rounded-xl p-4 cursor-pointer hover:border-red-300 transition-colors {{ (old('genero', $crente->genero) == 'feminino') ? 'border-red-500 bg-red-50' : '' }}"
                            onclick="selectRadioCard(this, 'genero')">
                            <input type="radio" name="genero" value="feminino" class="hidden"
                                {{ (old('genero', $crente->genero) == 'feminino') ? 'checked' : '' }}>
                            <div class="flex items-center">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        @php
                            $estadosCivis = [
                                'solteiro' => ['icon' => 'fa-user', 'color' => 'text-gray-600', 'label' => 'Solteiro(a)'],
                                'casado' => ['icon' => 'fa-ring', 'color' => 'text-red-600', 'label' => 'Casado(a)'],
                                'divorciado' => ['icon' => 'fa-user-times', 'color' => 'text-orange-600', 'label' => 'Divorciado(a)'],
                                'viuvo' => ['icon' => 'fa-heart-broken', 'color' => 'text-gray-600', 'label' => 'Viúvo(a)'],
                            ];
                        @endphp

                        @foreach($estadosCivis as $value => $data)
                            <label
                                class="border-2 border-gray-200 rounded-xl p-4 cursor-pointer text-center hover:border-red-300 transition-colors {{ (old('estado_civil', $crente->estado_civil) == $value) ? 'border-red-500 bg-red-50' : '' }}"
                                onclick="selectRadioCard(this, 'estado_civil')">
                                <input type="radio" name="estado_civil" value="{{ $value }}" class="hidden"
                                    {{ (old('estado_civil', $crente->estado_civil) == $value) ? 'checked' : '' }}>
        ...
        <div id="section2" class="bg-white rounded-xl shadow-sm border border-gray-100 mb-6 hidden">
            <div class="p-6 border-b border-gray-100">
                ...
                <p class="text-gray-600 text-sm mt-1">Edite datas importantes e status na igreja</p>
            </div>
                ...
                <div>
                    ...
                    <select name="grupo_id"
                        ...
                        @foreach ($grupos as $grupo)
                            <option value="{{ $grupo->id }}" {{ old('grupo_id', $crente->grupo_id) == $grupo->id ? 'selected' : '' }}>
                                {{ $grupo->nome }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="date" name="data_aceitacao"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition"
                                value="{{ old('data_aceitacao', $crente->data_aceitacao ? date('Y-m-d', strtotime($crente->data_aceitacao)) : '') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="data_batismo"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition"
                                value="{{ old('data_batismo', $crente->data_batismo ? date('Y-m-d', strtotime($crente->data_batismo)) : '') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="data_casamento"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition"
                                value="{{ old('data_casamento', $crente->data_casamento ? date('Y-m-d', strtotime($crente->data_casamento)) : '') }}">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border border-gray-200 p-4 rounded-lg">
                        <div class="flex items-center space-x-3">
                            <input type="checkbox" name="batizado" value="1"
                                ...
                                {{ old('batizado', $crente->batizado) ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center space-x-3">
                            <input type="checkbox" name="dizimista" value="1"
                                class="h-5 w-5 rounded border-gray-300 text-red-600 focus:ring-red-500"
                                {{ old('dizimista', $crente->dizimista) ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        @foreach ($estados as $value => $data)
                            <label
                                class="border-2 border-gray-200 rounded-xl p-4 cursor-pointer text-center hover:border-red-300 transition-colors {{ (old('estado', $crente->estado) == $value) ? 'border-red-500 bg-red-50' : '' }}"
                                onclick="selectRadioCard(this, 'estado')">
                                <input type="radio" name="estado" value="{{ $value }}" class="hidden"
                                    {{ (old('estado', $crente->estado) == $value) ? 'checked' : '' }}>
                                <i class="fas {{ $data['icon'] }} text-2xl {{ $data['color'] }} mb-2"></i>
                                <p class="font-medium">{{ $data['label'] }}</p>
                            </label>
                        @endforeach
...
@push('scripts')
    <script>
        ...
        // Inicializar cards selecionados com valores do banco
